...
def fix_mojibake_text(value):
    if not isinstance(value, str) or not value:
        return value
    if not any(m in value for m in ("Ã", "â", "Â", "ï»¿")):
        return value
    try:
...
class ReportesView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if export in ('csv', 'excel'):
            ...
            writer.writerow(['Ventas por categorÃ­a'])

            writer.writerow(['CategorÃ­a', 'Cantidad', 'Ingresos', 'Costo', 'Utilidad'])
            ...
            writer.writerow(['Citas por dÃ­a'])
            ...
            writer.writerow(['Consultas por dÃ­a'])
            ...
            writer.writerow(['Mascotas con mÃ¡s consultas'])

            writer.writerow(['Mascota', 'dueno', 'Consultas'])

            for row in mascotas_frecuentes:

                dueno = f"{row['mascota__owner__first_name']} {row['mascota__owner__last_name']}".strip()

                if not dueno:

                    dueno = row['mascota__owner__username']

                writer.writerow([row['mascota__nombre'], dueno, row['cantidad']])

            writer.writerow([])



            writer.writerow(['Stock crÃ­tico'])

            writer.writerow(['Producto', 'Stock', 'MÃ­nimo'])
...
class ReservaLoteCancelView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        InventoryMovement.objects.create(
            ...
            referencia=f"LiberaciÃ³n reserva #{reserva.id}",
    ...
    def form_valid(self, form):
        ...
        if seguimiento_fecha:
            ...
            if seguimiento_slot != seguimiento_fecha:
                ...
                messages.info(self.request, 'Horario ocupado. La cita de seguimiento se moviÃ³ al espacio mÃ¡s cercano disponible.')
            ...
            cita = Cita.objects.create(
                ...
                notas=f'Cita de seguimiento automÃ¡tico ({days} dÃ­as)'

            )

            log_cita_event(cita, CitaEvent.TIPO_CREADA, self.request.user, f'Seguimiento automÃ¡tico ({days} dÃ­as)')
...
class RecetaApproveView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if not receta.items.exists():
            ...
            messages.error(request, 'La receta no tiene Ã­tems para generar la venta.')
        ...
        messages.success(request, 'Receta aceptada. Se creÃ³ la venta y se descontÃ³ stock.')
...
class CitaCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # Si hay conflicto, buscar el espacio mÃ¡s cercano (30 min, hasta 24h)
        ...
        if obj.veterinario and obj.fecha:
            ...
            if slot != obj.fecha:
                ...
                messages.info(self.request, 'Horario ocupado. Se asignÃ³ la cita al espacio mÃ¡s cercano disponible.')
...
class CitaEventsView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for c in qs:
            ...
            title = fix_mojibake_text(c.mascota.nombre)

            if c.veterinario:

                title = f"{title} - {fix_mojibake_text(c.veterinario.nombre_completo)}"
...
class SolicitudCitaApproveView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if slot != dt:
            ...
            request.session['cita_asignada_msg'] = 'El horario solicitado estaba ocupado. Se asignÃ³ la cita al horario disponible mÃ¡s cercano.'
    ...
    def form_valid(self, form):
        ...
        if obj.nueva_fecha and self.cita.veterinario:
            ...
            if slot != obj.nueva_fecha:
                ...
                messages.info(self.request, 'Horario ocupado. Se propuso el espacio mÃ¡s cercano disponible para la reprogramaciÃ³n.')
...
class MascotaUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def has_permission(self):
        return super().has_permission() or has_role(self.request.user, ROLE_VETERINARIO)
